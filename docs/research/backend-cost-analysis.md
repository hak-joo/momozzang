...
### 6.4 케이스별 과금 예상 (2026-06-11 추가, 의사결정용)

> 모든 수치는 공식 가격 페이지 확인(확인일 2026-06-11). 과금은 트래픽에 비례하므로 아래 **공통 가정**을 깔고 트래픽 3구간으로 계산한다.

**공통 가정**
- 리사이즈 사진 평균 **400KB**, 청첩장당 콘텐츠 이미지 **~24장**(갤러리 20 + 메인/공유/신랑/신부) ≈ **10MB/청첩장**.
- 하객 1명이 청첩장을 끝까지 1회 조회 ≈ 10MB 전송 → **월 egress 10GB ≈ 1천 조회 / 100GB ≈ 1만 조회 / 500GB ≈ 5만 조회**.
- 과거 실측 기준점: Free에서 cached egress 5GB(≈500 조회) 초과로 서비스 중지([§3.4 Q4]).

**공식 한도·요율**

| | 무료/포함 한도 | 초과 요율 | 출처(확인일 2026-06-11) |
|---|---|---|---|
| Supabase Free | 저장 1GB, cached 5GB, uncached 5GB | (초과 시 차단) | https://supabase.com/pricing |
| Supabase Pro | $25/월, 저장 8GB, cached 250GB, uncached 250GB | 저장 $0.125/GB, cached $0.03/GB, uncached $0.09/GB. **Spend cap 기본 ON** | https://supabase.com/pricing |
| Firebase Blaze (신규 버킷) | 저장 5GB, 다운로드 100GB/월, 다운로드 작업 5만/월 | 저장 $0.026/GB, egress $0.12/GB, 작업 $0.004/만건 | https://firebase.google.com/pricing |
| R2 + 커스텀 도메인 | 저장 10GB, 읽기(Class B) 1,000만/월, 쓰기(Class A) 100만/월, **egress 무제한 무료** | 저장 $0.015/GB (egress·캐시 계속 무료) | https://developers.cloudflare.com/r2/pricing/ |

**케이스별 월 과금 (트래픽 3구간)**

| 월 트래픽 | ① Supabase Pro | ② Firebase Blaze | ③ R2 + 커스텀 도메인 |
|---|---|---|---|
| 10GB (≈1천 조회) | **$25** (포함 내) | **$0** | **$0.83** (도메인비만) |
| 100GB (≈1만 조회) | **$25** (포함 내) | **~$1** (egress 무료 경계, 작업 초과분) | **$0.83** |
| 500GB (≈5만 조회) | **~$32.5** ($25 + cached 초과 250GB×$0.03) | **~$48** (egress 400GB 초과×$0.12) | **$0.83** |

> 도메인비 = Cloudflare Registrar .com 연 ~$10의 월 환산. R2 자체는 무료.

**해석**
- **① Supabase Pro**: 250GB 안이면 트래픽 무관 **$25 고정**, spend cap으로 폭주 과금 없음. 단 트래픽이 적어도 매달 $25 고정비.
- **② Firebase Blaze**: 소규모는 거의 $0이나 **트래픽 늘수록 가장 비쌈**(egress GB당 과금 + CDN 캐싱 기본 미제공 + 다운로드 작업 5만/월 병목). read-heavy 청첩장에 구조적으로 부적합.
- **③ R2 + 도메인**: 트래픽 10GB든 500GB든 **월 ~$0.83 고정**. egress 무료라 조회 폭증과 비용이 분리됨.

**케이스 ③ 무료 한도 검증** (현실적 규모에서 걸리지 않음)

| R2 무료 한도 | 한도 | 현실적 소진 | 여유 |
|---|---|---|---|
| egress(전송) | 무제한 | — | ✅ 걸릴 수 없음 |
| 읽기 Class B | 1,000만/월 | Cloudflare 캐시 HIT는 R2 미접근. 캐시 없어도 500GB÷400KB=125만 | ✅ 8배 여유 |
| 쓰기 Class A | 100만/월 | 신청 폼 월 1,000개×24장=2.4만 | ✅ 40배 여유 |
| 저장 | 10GB | 청첩장 1개≈10MB → **약 1,000개**까지 무료 | ⚠️ 유일 한계점 |

→ 유일한 한계는 **저장 10GB(≈청첩장 1,000개)**. 초과해도 차단이 아니라 GB당 $0.015 종량(20GB여도 +$0.15/월). 오래된 청첩장 주기 정리 시 사실상 영구 무료.

**종합 권고**: 서비스로 키우면 **③ R2+도메인이 정답**(트래픽-비용 분리, 도메인은 신청 폼 브랜딩에 어차피 필요). 당장 결정을 미루려면 **① Supabase Pro $25/월**가 코드 변경 0의 즉시 해제 버튼. **② Firebase는 비추천**.

> **주의**: r2.dev 퍼블릭 도메인은 공식적으로 **비프로덕션·rate-limited·CDN 캐싱 불가**(출처: https://developers.cloudflare.com/r2/buckets/public-buckets/ , 확인일 2026-06-11). 따라서 ③은 **커스텀 도메인이 전제**다.
