// 3차 개정 시뮬레이션 (평가자 E3) — 계약 §0.8-P2 · 기준 39.
// preview-baseline.mjs 와 동일하되 `--f4` 를 주면 상단바를 한 행(73px)으로 강제해
// **F4 구현 후의 diff 가 어떤 키를 내는지**를 구현 전에 확인한다.
//
...
// ── F4 시뮬레이션: 상단바를 한 행으로 (Stepper .stepper{width:100%} 제거 + sticky offset 변수화)
if (process.argv.includes('--f4')) {
  await page.addStyleTag({ content: `
    nav[class*="stepper"] { width: auto !important; }
    [class*="previewPane"] { top: calc(73px + 16px) !important; }
  ` });
